...
export class ReportsService {
  ...
  private buildWhere(q: ReportQueryDto): Prisma.IssueWhereInput {
    const where: Prisma.IssueWhereInput = {};
    if (q.projectId != null) where.projectId = q.projectId;
    if (q.status && q.status !== 'All') where.status = q.status as IssueStatus;
    if (q.type && q.type !== 'All') where.type = q.type as IssueType;
  ...
  async executiveSummary(q: ReportQueryDto) {
  ...
  async byProject(q: ReportQueryDto) {
    const where = this.buildWhere(q);
    ...
    for (const row of grouped) {
      ...
      map[pid].total += c;
      const s = row.status;
      if (s === 'Open') map[pid].open += c;
      else if (s === 'InProgress') map[pid].inProgress += c;
      else if (s === 'InReview') map[pid].inReview += c;
      else if (s === 'Resolved') map[pid].resolved += c;
      else if (s === 'Closed') map[pid].closed += c;
      else if (s === 'Reopened') map[pid].reopened += c;
    }
  ...
  async byReporter(q: ReportQueryDto) {
    const where = this.buildWhere(q);
    const grouped = await this.prisma.issue.groupBy({
      by: ['reporterId'],
      where,
    ...
    const userIds = grouped.map((g) => g.reporterId).filter((id): id is string => id != null);
  ...
  async byAssignee(q: ReportQueryDto) {
    const where = this.buildWhere(q);
    const grouped = await this.prisma.issue.groupBy({
      by: ['assigneeId'],
      where,
    ...
    const empIds = grouped.map((g) => g.assigneeId).filter((id): id is string => id != null);
  ...
  async overdueAging(q: ReportQueryDto) {
  ...
  async issueRegister(q: ReportQueryDto) {
    const where = this.buildWhere(q);
  ...
  async weeklyTrend(q: ReportQueryDto) {
    const base = this.buildWhere(q);
    const start = new Date();
    start.setDate(start.getDate() - 7 * 16);
    start.setHours(0, 0, 0, 0);
    const merged: Prisma.IssueWhereInput = { ...base };
    const ca: Prisma.DateTimeFilter = { gte: start };
    if (base.createdAt && typeof base.createdAt === 'object' && !Array.isArray(base.createdAt)) {
      const b = base.createdAt as Prisma.DateTimeFilter;
      if (b.gte && b.gte > start) ca.gte = b.gte;
      if (b.lte) ca.lte = b.lte;
    }
    merged.createdAt = ca;
  ...
  async priorityMatrix(q: ReportQueryDto) {
    const where = this.buildWhere(q);
    const grouped = await this.prisma.issue.groupBy({
      by: ['priority', 'status'],
      where,
    ...
    const statuses = ['Open', 'InProgress', 'InReview', 'Resolved', 'Closed', 'Reopened'] as const;
    ...
    for (const row of grouped) {
      if (matrix[row.priority]) matrix[row.priority][row.status] = row._count._all;
    }
    return {
      generatedAt: new Date().toISOString(),
      filters: { projectId: q.projectId, dateFrom: q.dateFrom, dateTo: q.dateTo },
